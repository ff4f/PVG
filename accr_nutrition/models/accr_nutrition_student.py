...
class accrNutritionStudent(models.Model):
    ...
    student_medications = fields.One2many(
        related='student.x_medications', string=u'Medications', store=False, track_visibility='always', )
    ...
    student_leave_requests = fields.One2many(
        related="student.x_leave_requests", string=u"Leave Requests", store=False, readonly=False, )
    ...
    def onchange_leave_requests(self):
        cr.execute("""SELECT id FROM ir_model FWHERE model = %s""",
                   (str(self._name),))
